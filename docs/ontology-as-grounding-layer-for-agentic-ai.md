...
### 5. Supporting Evidence: LLM-Driven Ontology Construction

Recent research confirms that LLM-assisted ontology construction is not speculative — it is active, ongoing work with measurable results. The OntoEKG pipeline (Soru et al., ICSC 2026) demonstrated an LLM-driven approach that extracts classes and properties from unstructured enterprise text, structures them into a hierarchy via a separate entailment module, and serializes the result into standard RDF/Turtle.

Key findings relevant to this proposal:
- **Two-phase extraction + entailment mirrors our proposed workflow** — the LLM proposes, the system structures, the human validates. The paper achieves a fuzzy-match F1 of 0.724 in the Data domain, confirming that LLM-proposed ontologies are useful starting points requiring human oversight.
- **Identified limitations validate the need for human validation.** The authors report the LLM struggles with scope definition (what belongs vs. what doesn't), creates spurious subclass relationships, and confuses classes with instances — precisely the gaps a domain expert would catch during the validation step in our workflow.
- **The paper explicitly calls out the lack of benchmarks for end-to-end ontology construction** — supporting our earlier argument that ontology serves as its own normative reference.

The OntoEKG repository ([github.com/LiberAI/OntoEKG](https://github.com/LiberAI/OntoEKG)) provides an implementation reference for the MVO phase.
...
### 7. Key Objections and Rebuttals

#### 7.1 "Ontologies are too hard to build and maintain"
...
#### 7.2 "What happens when the ontology is wrong?"
...
#### 7.3 "This replaces statisticians"
...
#### 7.4 "What about regulatory acceptance?"
...
#### 7.5 "What about LLM-bootstrapping paradox?"
...
#### 8.2 Narrative Annotation: Preserving the "Why"
...
### 11. Known Unknowns and Failure Modes
...
### 12. Implementation-Level Considerations
...
### 13. What This Proposal Is and Isn't
